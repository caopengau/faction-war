...
Unit * Archer(int x, int y, Player * player){
	...
	archer->sight = 6;
	archer->cost = 10;
	if(x&&y&&(x-1!=player->x||y-1!=player->y)){
		archer->x = x-1;
		archer->y = y-1;
	}else{
...
void printunit(Unit * unit){
	printf("\t"); fillwith(86, '-'); printf("\n");
	printf("\t%5d%12s%6d%c%d%10d%14d/%2d/%2d%10d%10d%7d,%2d\n", 
		unit->player_id, unit->unit_type, unit->player_id, 
		unit->unit_symbol, unit->unit_id, unit->hitpoint, 
		unit->attack, unit->attackrange, unit->sight, unit->defense, unit->movement, 
		unit->x+1, unit->y+1);
}
/****************************************************************************/
...
void report(Player * p1){
	int i; Unit * unit;
	printf("\nResource: %d\n", p1->resource);
	printf("\t%5s%12s%8s%10s%20s%10s%10s%10s\n", "Side", "UnitType", 
			"Unitid", "Hitpoint", "Attack/range/sight", "Defense", "Movement", "Position");
